...
public class ThatPokedexAdmin {
	...
	public static List<String> allPokemon = new ArrayList<String>();
	public static HashMap<String, String> pokeHash = new HashMap<String, String>();
	public static String[] namesArray;
	public static String[] valueArray;
	private JTextField typeText;
	private JTextField typeGen;
	...
	public static void main(String[] args) throws FileNotFoundException {
		File pokemonFile = new File("sql.txt");
		Scanner scanFile = new Scanner(pokemonFile);
		...
		Set<String> keys = pokeHash.keySet();
		Collection<String> values = pokeHash.values();
		namesArray = keys.toArray(new String[keys.size()]);
		valueArray = values.toArray(new String[values.size()]);
	...
	private void initialize() {
		...
		frmThatPokedex.setBounds(100, 100, 911, 472);
		...
		DefaultListModel pokeList;
		pokeList = new DefaultListModel();
		
		JList list = new JList(pokeList);
		
		for(int i = 0; i<=namesArray.length-1;i++) {
			pokeList.addElement(namesArray[i]);
		}
		...
		*/
		...
		btnNewButton_1_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
					...
					else if(convert.equals("anorith")) {
						txtrInformationAboutThe.setFont(new Font("Tahoma", Font.BOLD, 19));
		...
		lblFilterBy.setBounds(663, 143, 104, 26);
		frmThatPokedex.getContentPane().add(lblFilterBy);
		
		JCheckBox typeCheck = new JCheckBox("Type");
		typeCheck.setBounds(601, 175, 93, 21);
		frmThatPokedex.getContentPane().add(typeCheck);
		
		typeText = new JTextField();
		typeText.setBounds(601, 207, 96, 19);
		frmThatPokedex.getContentPane().add(typeText);
		typeText.setColumns(10);
		
		JCheckBox genCheck = new JCheckBox("Generation");
		genCheck.setBounds(751, 175, 93, 21);
		frmThatPokedex.getContentPane().add(genCheck);
		
		typeGen = new JTextField();
		typeGen.setBounds(751, 207, 96, 19);
		frmThatPokedex.getContentPane().add(typeGen);
		typeGen.setColumns(10);
		
		JCheckBox sizeCheck = new JCheckBox("Size");
		sizeCheck.setBounds(601, 250, 93, 21);
		frmThatPokedex.getContentPane().add(sizeCheck);
		
		typeSize = new JTextField();
		typeSize.setBounds(601, 281, 96, 19);
		frmThatPokedex.getContentPane().add(typeSize);
		typeSize.setColumns(10);
		
		JCheckBox weightCheck = new JCheckBox("Weight");
		weightCheck.setBounds(751, 250, 93, 21);
		frmThatPokedex.getContentPane().add(weightCheck);
		
		textField_3 = new JTextField();
		textField_3.setBounds(751, 281, 96, 19);
		frmThatPokedex.getContentPane().add(textField_3);
		...
		btnNewButton_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//TYPE FILTER
				if(typeCheck.isSelected() == true && typeText.getText() != null) {
				for(int i = 0; i<=pokeHash.size()-1; i++) {					
					if(getData(allPokemon.get(i), 1).toLowerCase().contains(typeText.getText().toLowerCase())) {
						combineList.add(getData(allPokemon.get(i), 0));
					}
				}
				}
				...
				//GEN FILTER
				else if(genCheck.isSelected() == true && typeGen.getText() != null) {
				for(int i = 0; i<=pokeHash.size()-1; i++) {					
					if(getData(allPokemon.get(i), 2).contains(typeGen.getText())) {
						combineList.add(getData(allPokemon.get(i), 0));
					}
				}
				}
				...
				/*
				//SIZE FILTER
				else if(genCheck.isSelected() == true && typeSize.getText() != null) {
				for(int i = 0; i<=pokeHash.size()-1; i++) {					
					if(getData(allPokemon.get(i), 3).contains(typeSize.getText())) {
						combineList.add(getData(allPokemon.get(i), 0));
					}
				}
				}
				*/
				...
				for(int i = 0; i<=combineList.size()-1;i++) {
					pokeList.addElement(combineList.get(i));
				}
			}
		});
		btnNewButton_2.setBounds(633, 343, 85, 21);
		frmThatPokedex.getContentPane().add(btnNewButton_2);
		...
		btnNewButton_3.setBounds(751, 343, 85, 21);
		frmThatPokedex.getContentPane().add(btnNewButton_3);
	}
	
	//PARSE FUNCTIONS, NEED TO MAKE INTO CLASS
	
	public static void addPokeToHash(String pokeLine) {
		String[] data = pokeLine.split(";", 2);
		pokeHash.put(data[0], pokeLine);
	}
	
	public static String getData(String pokeLine, int index) {
		String[] data = pokeLine.split(";", 5);
		return data[index];
	}
}
